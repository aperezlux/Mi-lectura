...
import { formatDate, getLiturgicalSeason, checkProximityConflict, cn, ensureArray, apiUrl } from "@/lib/utils";
...
function ConfigTab() {
  ...
  const loadConfig = async () => {
    try {
      ...
      setConfigState({
        generationConfigurations: Array.isArray(data.generationConfigurations) ? data.generationConfigurations : [],
        liturgicalFunctions: Array.isArray(data.liturgicalFunctions) ? data.liturgicalFunctions : [],
        functionAssignments: Array.isArray(data.functionAssignments) ? data.functionAssignments : [],
  ...
  const saveConfig = async () => {
    try {
      ...
      const safeAssignments = (configState.functionAssignments ?? []).filter((assignment) => {
        const functionId = Number(assignment.functionId);
        return Number.isFinite(functionId) && functionId > 0 && configState.liturgicalFunctions.some((fn) => Number(fn.id) === functionId);
      });
      const response = await fetch(apiUrl("/api/config"), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...configState, functionAssignments: safeAssignments }),
      });
  ...
  const updateFunction = (functionId: number, patch: Record<string, any>) => {
    setConfigState((prev) => ({
      ...prev,
      liturgicalFunctions: prev.liturgicalFunctions.map((fn) => (Number(fn.id) === Number(functionId) ? { ...fn, ...patch } : fn)),
    }));
  };

  const toggleAssignment = (generationConfigurationId: number, functionId: number) => {
    setConfigState((prev) => {
      const exists = prev.functionAssignments.some((assignment) => Number(assignment.generationConfigurationId) === generationConfigurationId && Number(assignment.functionId) === functionId);
      if (exists) {
        return {
          ...prev,
          functionAssignments: prev.functionAssignments.filter((assignment) => !(Number(assignment.generationConfigurationId) === generationConfigurationId && Number(assignment.functionId) === functionId)),
        };
      }
      return {
        ...prev,
        functionAssignments: [
          ...prev.functionAssignments,
          {
            generationConfigurationId,
            functionId,
            sortOrder: prev.functionAssignments.filter((assignment) => Number(assignment.generationConfigurationId) === generationConfigurationId).length + 1,
            isActive: true,
          },
        ],
      };
  ...
  const addFunction = () => {
    ...
    const newFunction = {
      ...
      isVisible: true,
      requiresReader: true,
      sortOrder: configState.liturgicalFunctions.length + 1,
      color: "#0f766e",
      icon: "✨",
      description: "",
    };
    setConfigState((prev) => ({ ...prev, liturgicalFunctions: [...prev.liturgicalFunctions, newFunction] }));
    setNewFunctionName("");
  };

  const updateAppSetting = (key: string, value: boolean) => {
    setConfigState((prev) => ({ ...prev, appSettings: { ...prev.appSettings, [key]: value ? "true" : "false" } }));
  };

  if (isLoading || configLoading) return <div className="py-12 flex justify-center"><div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>;

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl font-serif text-primary flex items-center gap-2">
          <Settings className="w-6 h-6 text-accent" /> Configuración de Horarios y Generación
        </h2>
        <p className="text-muted-foreground mt-2 text-sm">
          Ajusta los horarios, los roles que se generan por tipo de día y si los lectores pueden marcar sus días no disponibles desde su portal.
        </p>
      </div>

      <div className="flex items-center justify-end">
        <Button size="sm" onClick={saveConfig} disabled={savingConfig}>{savingConfig ? "Guardando..." : "Guardar configuración"}</Button>
      </div>

      <Card className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-primary">Portal de disponibilidad del lector</h3>
            <p className="text-sm text-muted-foreground">Controla si los lectores pueden ver y usar el calendario de indisponibilidad desde su portal.</p>
          </div>
          <button
            onClick={() => updateAppSetting("reader_availability_visible", configState.appSettings.reader_availability_visible !== "true")}
            className={cn("mt-1 shrink-0 w-12 h-7 rounded-full transition-colors border relative", configState.appSettings.reader_availability_visible === "true" ? "bg-primary/20 border-primary/40" : "bg-muted border-border")}
          >
            <span className={cn("absolute top-1 left-1 w-5 h-5 rounded-full transition-transform", configState.appSettings.reader_availability_visible === "true" ? "bg-primary translate-x-5" : "bg-muted-foreground")} />
          </button>
        </div>
      </Card>
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-primary">Funciones litúrgicas</h3>
            <p className="text-sm text-muted-foreground">Define qué funciones pueden aparecer al generar el calendario.</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Input value={newFunctionName} onChange={(e) => setNewFunctionName(e.target.value)} placeholder="Nueva función" />
          <Button size="sm" onClick={addFunction}>Agregar</Button>
        </div>
        <div className="space-y-3">
          {configState.liturgicalFunctions.map((fn: any) => (
            <div key={fn.id} className="rounded-xl border border-border p-3 space-y-3">
              <div className="flex items-center gap-2">
                <Input value={fn.name ?? ""} onChange={(e) => updateFunction(fn.id, { name: e.target.value })} className="h-10" />
                <Button size="sm" variant={fn.isActive ? "primary" : "ghost"} onClick={() => updateFunction(fn.id, { isActive: !fn.isActive })}>
                  {fn.isActive ? "Activa" : "Inactiva"}
                </Button>
              </div>
              <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={fn.isVisible ?? true} onChange={(e) => updateFunction(fn.id, { isVisible: e.target.checked })} />
                  Visible
                </label>
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={fn.requiresReader ?? true} onChange={(e) => updateFunction(fn.id, { requiresReader: e.target.checked })} />
                  Requiere lector
                </label>
              </div>
            </div>
      ...
      <Card className="p-5 space-y-4">
        <div>
          <h3 className="font-semibold text-primary">Roles por tipo de día</h3>
          <p className="text-sm text-muted-foreground">Marca qué funciones se usarán para cada tipo de misa al generar el calendario.</p>
        </div>
        <div className="space-y-4">
          {configState.generationConfigurations.map((config: any) => {
            const assignedIds = new Set((configState.functionAssignments ?? [])
              .filter((assignment) => Number(assignment.generationConfigurationId) === Number(config.id))
              .map((assignment) => Number(assignment.functionId)));
            return (
              <div key={config.id} className="rounded-xl border border-border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-semibold">{config.label}</h4>
                    <p className="text-sm text-muted-foreground">{config.dayType}</p>
                  </div>
                  <button
                    onClick={() => {
                      setConfigState((prev) => ({
                        ...prev,
                        generationConfigurations: prev.generationConfigurations.map((entry) => Number(entry.id) === Number(config.id) ? { ...entry, isEnabled: !entry.isEnabled } : entry),
                      }));
                    }}
                    className={cn("shrink-0 w-10 h-6 rounded-full transition-colors border relative", config.isEnabled ? "bg-primary/20 border-primary/40" : "bg-muted border-border")}
                  >
                    <span className={cn("absolute top-1 left-1 w-4 h-4 rounded-full transition-transform", config.isEnabled ? "bg-primary translate-x-4" : "bg-muted-foreground")} />
                  </button>
                </div>
                <div className="flex flex-wrap gap-3">
                  {configState.liturgicalFunctions.map((fn: any) => (
                    <label key={fn.id} className="flex items-center gap-2 text-sm">
                      <input type="checkbox" checked={assignedIds.has(Number(fn.id))} onChange={() => toggleAssignment(Number(config.id), Number(fn.id))} />
                      {fn.name}
                    </label>
                  ))}
                </div>
      ...
      <Card className="p-4 space-y-3">
        <h3 className="font-semibold text-primary">Horarios</h3>
        <div className="space-y-3">
          {schedules.map((s: MassSchedule) => (
            <div key={s.id} className={cn("rounded-xl border border-border p-4 transition-opacity", !s.isActive && "opacity-60")}>
